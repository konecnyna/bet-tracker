...
String.prototype.inProgress = function () {
  return !["PREGAME", "FINAL", "FINAL_OVERTIME"].includes(this.toString())
}
...
function addSpreadData(games, picks) {
  for (var i = 0; i < games.length; i++) {
    ...
    if (isBetTeamHome) {
      ...
      currentGame.homeSpread = spread.toString();
    } else {
      currentGame.vnn = currentGame.vnn + " (" + spread + ")";
      currentGame.awaySpread = spread.toString();
    }
    ...
    if (currentGame.q !== "PREGAME" && currentGame.q !== "P") {
      if (covering > 0) {
        currentGame.covering_text = !currentGame.q.inProgress()
          ? "Covered"
