...
function addSpreadData(currentGame, pick) {
  ...
  if (isBetTeamHome) {
    currentGame.hdisplay = currentGame.hdisplay + " (" + spread + ")";
  } else {
    currentGame.vdisplay = currentGame.vdisplay + " (" + spread + ")";
  }
  ...
  if (currentGame.q === "HALFTIME") {
    ...
    currentGame.time_text = `${daysOfWeek[gameDate.getDay()]} at ${gameDate.getHours() > 12 ? gameDate.getHours() - 12 : gameDate.getHours()}:${gameDate.getMinutes() == 0 ? "00" : gameDate.getMinutes()} ${gameDate.getHours() > 12 ? "PM" : "AM"}`;
  }

  if (currentGame.v === "LV" || currentGame.h === "LV") {
