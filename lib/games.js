...
function addSpreadData(games, picks) {
  for (var i = 0; i < games.length; i++) {
    ...
    if (currentGame.q !== "PREGAME") {
      if (covering > 0) {
        currentGame.covering_text = currentGame.q.match(/F/)
          ? "Covered"
          : "Covering!";
    ...
    if (currentGame.q === "HALFTIME") {
      ...
      const tweleveHrTime = gameDate.toLocaleTimeString().split(":")
      currentGame.time_text =  `${daysOfWeek[gameDate.getDay()]} at ${tweleveHrTime[0]}:${tweleveHrTime[1]} ${tweleveHrTime[2].includes("PM") ? "PM" : "AM"}`;
    }
