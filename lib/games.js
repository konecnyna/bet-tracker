...
function getGameData(data, picks) {
  ...
  for (var j = 0; j < picks.length; j++) {
    ...
    console.log(name, currentPick)
    const game = data.find(({ awayTeam, homeTeam }) => {
      console.log(homeTeam.name, awayTeam.name)
      // return visitorTeam.name.toLowerCase().includes(name.toLowerCase())
...
function createLegacyObject(game) {
  return {
    d: "",
    t: game.time || "",
    q: game.quater || game.status,
    hnn: game.awayTeam.name,
    hs: game.homeTeam.score,
    v: game.awayTeam.name,
    h: game.homeTeam.name,
    vnn: game.homeTeam.name,
